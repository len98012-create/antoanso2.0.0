...
const App: React.FC = () => {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: 'welcome',
      role: Role.MODEL,
      text: 'Xin chào! Tớ là **Cố vấn An toàn Số**. \n\nTớ ở đây để giúp cậu:\n- 🛡️ Bảo mật thông tin cá nhân\n- 🚫 Phòng tránh lừa đảo trực tuyến\n- 🧠 Kiểm tra kiến thức qua các bài Quiz thú vị!\n\nCậu đang quan tâm đến vấn đề gì thế?',
      timestamp: new Date(),
    },
  ]);
  ...
  const handleStartQuiz = async (config: QuizConfig) => {
    ...
    try {
      ...
      setIsQuizConfigOpen(false); // Close config, open player (via quizData check)
    } catch (error) {
